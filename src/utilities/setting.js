...
export const updateDomValueBySetting = (
  setting,
  Choices
) => {
  updateHtmlAttr({
    prop: "dir",
    value: setting.theme_scheme_direction.value,
  });
  updateThemeScheme(
  ...
  updateBodyClass(
    Choices.Animation,
    setting.theme_transition.value
  );
  updateTitle(setting.app_name.value);
  setFontFamily(
    "body",
    setting.body_font_family.value
  );
  setFontFamily(
    "heading",
    setting.heading_font_family.value
  );
};
...
export const updateThemeScheme = (
  ...
  const filterValue = className.filter(
    (x) => x !== "auto"
  );
  updateBodyClass(
